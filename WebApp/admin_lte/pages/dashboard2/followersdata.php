<?php
    ...
    if (isset($_POST['time_start'])){
        ...
        foreach ($result as $row) {
            ...
            // $productName = "ice2";   // Testing icecreams
            // $icecream = "ice2";
            ...
            if ($class_id == '4'){
                ...
                if(strpos($row['product_name'], "ice1") !== false){
                    // If ice1 found:
                    $iceOne = ++$iceOne;
                } //else{

                // if(strpos($row['product_name'], "ice2") !== false){
                    if(strpos($row['product_name'], "ice2") !== false){
                        // If ice1 found:
                    $iceTwo = ++$iceTwo;
                }// else{
                ...
                if(strpos($row['product_name'], "ice3") !== false){
                            // If ice1 found:
                    $iceThree = ++$iceThree;
                }// else{
                            // Other class 4 products here:
        ...
        */
        ...
        // Send all data back
        $post_data = json_encode(array('labels' => $labels, 'data_cups' => $data_cups, 'data_syrops' => $data_syrops, 'coffee' => $coffee, 'milk' => $milk, 'syrop' => $syrop, 'price' => $price, 'table' => $data_table, 'data_pieChart' => $php_data_array, 'iceOne' => $iceOne, 'iceTwo' => $iceTwo, 'iceThree' =>$iceThree ));
